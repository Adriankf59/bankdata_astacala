                    ...
                    {columns.map((col) => (
                      <td key={col.key} className="px-6 py-4 text-sm text-gray-300">
                        {col.key === 'jalur_pda' && row[col.key] ? (
                          ...
                          >
                            <i className="fas fa-route mr-1 text-xs"></i>
                            <span className="text-xs font-medium">Lihat Jalur</span>
                          </a>
                        ) : col.key === 'point_pda' && row[col.key] ? (
                          <a 
                            href={`#${row[col.key]}`}
                          ...
                          >
                            ...
                            <span className="text-xs font-medium">Lihat Point</span>
                          </a>
  ...
  if (type === 'diksar' && pdaData && pdaData.data && pdaData.data.length > 0) {
    activity.table = {
      columns: [
        ...
        { key: 'jalur_pda', label: 'Jalur PDA' },
        { key: 'point_pda', label: 'Point PDA' },
      ],
      rows: pdaData.data
    };
  }

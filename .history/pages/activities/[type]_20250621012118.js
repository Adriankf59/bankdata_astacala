                  ...
                  >
                    {columns.map((col) => (
                      <td key={col.key} className="px-6 py-4 text-sm text-gray-300">
                        ...
                        // START: Penambahan Handler untuk Peta PDA
                        ...
                        // END: Penambahan Handler untuk Peta PDA
...
export default function ActivityDetail({ type, astacalaData, externalData, issData, pdaData, oradData }) {
  // ... (sisa komponen ActivityDetail tetap sama) ...
...
          _B41_A: {type === 'diksar' ? 'Belum ada data kegiatan PDA' : type === 'rafting' ? 'Belum ada data kegiatan ORAD' : 'No location data available'}
                </p>
...
  if (type === 'rafting') {
    try {
      // Fetch ORAD data from the new endpoint

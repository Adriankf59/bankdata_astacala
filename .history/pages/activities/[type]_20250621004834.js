  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        <input
          type="text"
          placeholder="Cari kegiatan ORAD..."
          value={searchTerm}
          onChange={(e) => {
                  ...
                  >
                    {columns.map((col) => (
                      <td key={col.key} className="px-6 py-4 text-sm text-gray-300">
                        ...
                        ) : (col.key === 'tempat' || col.key === 'tanggal_kegiatan') && row[col.key] ? (
                          <div className="relative">
                            <div className={`${!expandedDescriptions[idx] && col.key === 'tempat' ? 'max-w-[300px] truncate' : ''}`}>
                              {row[col.key]}
                            </div>
                            {row[col.key].length > 50 && col.key === 'tempat' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();

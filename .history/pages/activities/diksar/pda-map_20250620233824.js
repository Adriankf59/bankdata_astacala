...
const PDAMapPage = ({ routeData, pointData, nama, error }) => {
  ...
  // Initialize map
  useEffect(() => {
    if (!mapContainerRef.current || !routeData || !pointData) return;
  ...
  useEffect(() => {
    if (map && map.loaded() && routeData && pointData) {
      addDataToMap(map);
    }
  ...
  const addDataToMap = (mapInstance) => {
    ...
    const cleanupExistingLayers = () => {
      ...
      // Remove existing point layers and sources
  ...
  const changeMapStyle = (newStyle) => {
    ...
    if (map) {
      ...
      map.once('style.load', () => {
        ...
        // Re-add terrain if 3D was enabled
        if (was3D) {
          map.addSource('terrain', {
            type: 'raster-dem',
            url: `https://api.maptiler.com/tiles/terrain-rgb-v2/tiles.json?key=${key}`

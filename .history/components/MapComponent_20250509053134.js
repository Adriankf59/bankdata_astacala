...
const MapComponent = () => {
  ...
  const [cavingData, setCavingData] = useState([]); // State for cave data
  ...
  const fetchCaveData = async () => {
    try {
      ...
      const transformedData = data.data.map(cave => {
      ...
      setCavingData(transformedData);
    } catch (error) {
      console.error("Error fetching cave data:", error);
    }
  };

  // Fetch cave data when component mounts
  useEffect(() => {
    fetchCaveData();
  }, []);

  // Update points when cave data changes
  useEffect(() => {
